...
// TableName overrides the default tablename generated by GORM
func (BranchORM) TableName() string {
	return "branches"
}

// ToORM runs the BeforeToORM hook if present, converts the fields of this
// object to ORM format, runs the AfterToORM hook, then returns the ORM object
func (m *Branch) ToORM(ctx context.Context) (BranchORM, error) {
	to := BranchORM{}
	var err error
	if prehook, ok := interface{}(m).(BranchWithBeforeToORM); ok {
		if err = prehook.BeforeToORM(ctx, &to); err != nil {
			return to, err
		}
	}
	...
	if posthook, ok := interface{}(m).(BranchWithAfterToORM); ok {
		err = posthook.AfterToORM(ctx, &to)
	}
	return to, err
}
...
// ToPB runs the BeforeToPB hook if present, converts the fields of this
// object to PB format, runs the AfterToPB hook, then returns the PB object
func (m *BranchORM) ToPB(ctx context.Context) (Branch, error) {
	to := Branch{}
	var err error
	if prehook, ok := interface{}(m).(BranchWithBeforeToPB); ok {
		if err = prehook.BeforeToPB(ctx, &to); err != nil {
			return to, err
		}
	}
	...
	if posthook, ok := interface{}(m).(BranchWithAfterToPB); ok {
		err = posthook.AfterToPB(ctx, &to)
	}
	return to, err
}
...
// The following are interfaces you can implement for special behavior during ORM/PB conversions
// of type Branch the arg will be the target, the caller the one being converted from

// BranchBeforeToORM called before default ToORM code
type BranchWithBeforeToORM interface {
	BeforeToORM(context.Context, *BranchORM) error
}

// BranchAfterToORM called after default ToORM code
type BranchWithAfterToORM interface {
	AfterToORM(context.Context, *BranchORM) error
}

// BranchBeforeToPB called before default ToPB code
type BranchWithBeforeToPB interface {
	BeforeToPB(context.Context, *Branch) error
}

// BranchAfterToPB called after default ToPB code
type BranchWithAfterToPB interface {
	AfterToPB(context.Context, *Branch) error
}
...
// DefaultCreateBranch executes a basic gorm create call
func DefaultCreateBranch(ctx context.Context, in *Branch, db *gorm.DB) (*Branch, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(BranchORMWithBeforeCreate_); ok {
		if db, err = hook.BeforeCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(BranchORMWithAfterCreate_); ok {
		if err = hook.AfterCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
...
type BranchORMWithBeforeCreate_ interface {
	BeforeCreate_(context.Context, *gorm.DB) (*gorm.DB, error)
}
type BranchORMWithAfterCreate_ interface {
	AfterCreate_(context.Context, *gorm.DB) error
}

func DefaultReadBranch(ctx context.Context, in *Branch, db *gorm.DB) (*Branch, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(BranchORMWithBeforeReadApplyQuery); ok {
		if db, err = hook.BeforeReadApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	if db, err = gorm1.ApplyFieldSelection(ctx, db, nil, &BranchORM{}); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(BranchORMWithBeforeReadFind); ok {
		if db, err = hook.BeforeReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
	ormResponse := BranchORM{}
	if err = db.Where(&ormObj).First(&ormResponse).Error; err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormResponse).(BranchORMWithAfterReadFind); ok {
		if err = hook.AfterReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
...
type BranchORMWithBeforeReadApplyQuery interface {
	BeforeReadApplyQuery(context.Context, *gorm.DB) (*gorm.DB, error)
}
type BranchORMWithBeforeReadFind interface {
	BeforeReadFind(context.Context, *gorm.DB) (*gorm.DB, error)
}
type BranchORMWithAfterReadFind interface {
	AfterReadFind(context.Context, *gorm.DB) error
}

func DefaultDeleteBranch(ctx context.Context, in *Branch, db *gorm.DB) error {
	if in == nil {
		return errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(BranchORMWithBeforeDelete_); ok {
		if db, err = hook.BeforeDelete_(ctx, db); err != nil {
			return err
		}
	}
	err = db.Where(&ormObj).Delete(&BranchORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := interface{}(&ormObj).(BranchORMWithAfterDelete_); ok {
		err = hook.AfterDelete_(ctx, db)
	}
	return err
}

type BranchORMWithBeforeDelete_ interface {
	BeforeDelete_(context.Context, *gorm.DB) (*gorm.DB, error)
}
type BranchORMWithAfterDelete_ interface {
	AfterDelete_(context.Context, *gorm.DB) error
}

func DefaultDeleteBranchSet(ctx context.Context, in []*Branch, db *gorm.DB) error {
	if in == nil {
		return errors.NilArgumentError
	}
	var err error
	...
	if hook, ok := (interface{}(&BranchORM{})).(BranchORMWithBeforeDeleteSet); ok {
		if db, err = hook.BeforeDeleteSet(ctx, in, db); err != nil {
			return err
		}
	}
	err = db.Where("id in (?)", keys).Delete(&BranchORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := (interface{}(&BranchORM{})).(BranchORMWithAfterDeleteSet); ok {
		err = hook.AfterDeleteSet(ctx, in, db)
	}
	return err
}

type BranchORMWithBeforeDeleteSet interface {
	BeforeDeleteSet(context.Context, []*Branch, *gorm.DB) (*gorm.DB, error)
}
type BranchORMWithAfterDeleteSet interface {
	AfterDeleteSet(context.Context, []*Branch, *gorm.DB) error
}

// DefaultStrictUpdateBranch clears / replaces / appends first level 1:many children and then executes a gorm update call
func DefaultStrictUpdateBranch(ctx context.Context, in *Branch, db *gorm.DB) (*Branch, error) {
	if in == nil {
		return nil, fmt.Errorf("Nil argument to DefaultStrictUpdateBranch")
	}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	...
	lockedRow := &BranchORM{}
	db.Model(&ormObj).Set("gorm:query_option", "FOR UPDATE").Where("id=?", ormObj.Id).First(lockedRow)
	if hook, ok := interface{}(&ormObj).(BranchORMWithBeforeStrictUpdateCleanup); ok {
		if db, err = hook.BeforeStrictUpdateCleanup(ctx, db); err != nil {
			return nil, err
		}
	}
	if hook, ok := interface{}(&ormObj).(BranchORMWithBeforeStrictUpdateSave); ok {
		if db, err = hook.BeforeStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(BranchORMWithAfterStrictUpdateSave); ok {
		if err = hook.AfterStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
...
type BranchORMWithBeforeStrictUpdateCleanup interface {
	BeforeStrictUpdateCleanup(context.Context, *gorm.DB) (*gorm.DB, error)
}
type BranchORMWithBeforeStrictUpdateSave interface {
	BeforeStrictUpdateSave(context.Context, *gorm.DB) (*gorm.DB, error)
}
type BranchORMWithAfterStrictUpdateSave interface {
	AfterStrictUpdateSave(context.Context, *gorm.DB) error
}

// DefaultPatchBranch executes a basic gorm update call with patch behavior
func DefaultPatchBranch(ctx context.Context, in *Branch, updateMask *field_mask.FieldMask, db *gorm.DB) (*Branch, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	var pbObj Branch
	var err error
	if hook, ok := interface{}(&pbObj).(BranchWithBeforePatchRead); ok {
		if db, err = hook.BeforePatchRead(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	pbReadRes, err := DefaultReadBranch(ctx, &Branch{Id: in.GetId()}, db)
	if err != nil {
		return nil, err
	}
	pbObj = *pbReadRes
	if hook, ok := interface{}(&pbObj).(BranchWithBeforePatchApplyFieldMask); ok {
		if db, err = hook.BeforePatchApplyFieldMask(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	if _, err := DefaultApplyFieldMaskBranch(ctx, &pbObj, in, updateMask, "", db); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&pbObj).(BranchWithBeforePatchSave); ok {
		if db, err = hook.BeforePatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	pbResponse, err := DefaultStrictUpdateBranch(ctx, &pbObj, db)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(pbResponse).(BranchWithAfterPatchSave); ok {
		if err = hook.AfterPatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
...
type BranchWithBeforePatchRead interface {
	BeforePatchRead(context.Context, *Branch, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type BranchWithBeforePatchApplyFieldMask interface {
	BeforePatchApplyFieldMask(context.Context, *Branch, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type BranchWithBeforePatchSave interface {
	BeforePatchSave(context.Context, *Branch, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type BranchWithAfterPatchSave interface {
	AfterPatchSave(context.Context, *Branch, *field_mask.FieldMask, *gorm.DB) error
}

// DefaultPatchSetBranch executes a bulk gorm update call with patch behavior
func DefaultPatchSetBranch(ctx context.Context, objects []*Branch, updateMasks []*field_mask.FieldMask, db *gorm.DB) ([]*Branch, error) {
	if len(objects) != len(updateMasks) {
		return nil, fmt.Errorf(errors.BadRepeatedFieldMaskTpl, len(updateMasks), len(objects))
	}

	results := make([]*Branch, 0, len(objects))
	for i, patcher := range objects {
		pbResponse, err := DefaultPatchBranch(ctx, patcher, updateMasks[i], db)
		if err != nil {
			return nil, err
		}
...
// DefaultApplyFieldMaskBranch patches an pbObject with patcher according to a field mask.
func DefaultApplyFieldMaskBranch(ctx context.Context, patchee *Branch, patcher *Branch, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Branch, error) {
	if patcher == nil {
		return nil, nil
	} else if patchee == nil {
		return nil, errors.NilArgumentError
...
// DefaultListBranch executes a gorm list call
func DefaultListBranch(ctx context.Context, db *gorm.DB) ([]*Branch, error) {
	in := Branch{}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(BranchORMWithBeforeListApplyQuery); ok {
		if db, err = hook.BeforeListApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	db, err = gorm1.ApplyCollectionOperators(ctx, db, &BranchORM{}, &Branch{}, nil, nil, nil, nil)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(BranchORMWithBeforeListFind); ok {
		if db, err = hook.BeforeListFind(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(BranchORMWithAfterListFind); ok {
		if err = hook.AfterListFind(ctx, db, &ormResponse); err != nil {
			return nil, err
		}
	}
...
type BranchORMWithBeforeListApplyQuery interface {
	BeforeListApplyQuery(context.Context, *gorm.DB) (*gorm.DB, error)
}
type BranchORMWithBeforeListFind interface {
	BeforeListFind(context.Context, *gorm.DB) (*gorm.DB, error)
}
type BranchORMWithAfterListFind interface {
	AfterListFind(context.Context, *gorm.DB, *[]BranchORM) error
}
