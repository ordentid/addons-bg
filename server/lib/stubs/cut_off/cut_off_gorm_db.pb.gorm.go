...
type CutOffORM struct {
	...
	LastApprovedByID    uint64     `gorm:"column:created_at"`
	LastRejectedByID    uint64     `gorm:"column:created_at"`
	ModuleID            string     `gorm:"column:Modul_id;type:jsonb"`
	RepeatOn            string     `gorm:"column:repeat_on;type:jsonb"`
	RepeatPattern       string     `gorm:"column:repeat_pattern"`
	Saved               bool       `gorm:"column:Saved"`
	ScheduleDescription string     `gorm:"column:schedule_description"`
	ScheduleID          uint64     `gorm:"column:Schedule_id;primary_key;not null"`
	ScheduleName        string     `gorm:"column:schedule_name"`
	ScheduleTime        string     `gorm:"column:schedule_time;type:jsonb"`
