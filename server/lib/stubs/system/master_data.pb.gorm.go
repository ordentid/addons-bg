...
type MdBicccORM struct {
	...
	Isrma       bool   `gorm:"column:is_rma;default:false;not null"`
...
func (m *MdBranch) ToORM(ctx context.Context) (MdBranchORM, error) {
	...
	to.Bibr = m.Bibr
	to.Alamat = m.Alamat
	to.Provinsi = m.Provinsi
	to.Kabupaten = m.Kabupaten
	to.Kecamatan = m.Kecamatan
	to.Kelurahan = m.Kelurahan
	to.Kodepos = m.Kodepos
...
func DefaultApplyFieldMaskMdBranch(ctx context.Context, patchee *MdBranch, patcher *MdBranch, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*MdBranch, error) {
	...
	for _, f := range updateMask.Paths {
		...
		if f == prefix+"Bibr" {
			patchee.Bibr = patcher.Bibr
			continue
		}
		if f == prefix+"Alamat" {
			patchee.Alamat = patcher.Alamat
			continue
		}
		if f == prefix+"Provinsi" {
			patchee.Provinsi = patcher.Provinsi
			continue
		}
		if f == prefix+"Kabupaten" {
			patchee.Kabupaten = patcher.Kabupaten
			continue
		}
		if f == prefix+"Kecamatan" {
			patchee.Kecamatan = patcher.Kecamatan
			continue
		}
		if f == prefix+"Kelurahan" {
			patchee.Kelurahan = patcher.Kelurahan
			continue
		}
		if f == prefix+"Kodepos" {
			patchee.Kodepos = patcher.Kodepos
			continue
		}
	}
	if err != nil {
